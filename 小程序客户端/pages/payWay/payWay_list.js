...
Page({
  ...
  listPayWay: function () {
    var self = this
    var url = config.basePath + "api/payWay/list"
    //如果要显示的页码超过总页码不操作
    ...
    utils.sendRequest(url, {
      ...
      setTimeout(function () {  
        self.setData({
          payWays: self.data.payWays.concat(res.data.list),
  ...
  //删除支付方式记录
  removePayWay: function (e) {
    var self = this
    var payWayId = e.currentTarget.dataset.paywayid
    wx.showModal({
      title: '提示',
      content: '确定要删除payWayId=' + payWayId + '的记录吗？',
      success: function (sm) {
        if (sm.confirm) {
          // 用户点击了确定 可以调用删除方法了
          var url = config.basePath + "api/payWay/delete/" + payWayId
          utils.sendRequest(url, null, function (res) {
            ...
            //删除支付方式后客户端同步删除数据
            var payWays = self.data.payWays;
            for (var i = 0; i < payWays.length; i++) {
              if (payWays[i].payWayId == payWayId) {
                payWays.splice(i, 1)
                break
              }
            }
            self.setData({ payWays: payWays })
          })
        } else if (sm.cancel) {
          console.log('用户点击取消')
        }
