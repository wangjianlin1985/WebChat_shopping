...
Page({
  ...
  // 加载订单状态列表
  listOrderState: function () {
    var self = this
    var url = config.basePath + "api/orderState/list"
    //如果要显示的页码超过总页码不操作
    ...
    utils.sendRequest(url, {
      ...
      setTimeout(function () {  
        self.setData({
          orderStates: self.data.orderStates.concat(res.data.list),
  ...
  removeOrderState: function (e) {
    ...
    wx.showModal({
      ...
      success: function (sm) {
        if (sm.confirm) {
          // 用户点击了确定 可以调用删除方法了
          var url = config.basePath + "api/orderState/delete/" + stateId
          utils.sendRequest(url, null, function (res) {
            ...
            //删除订单状态后客户端同步删除数据
            var orderStates = self.data.orderStates;
            for (var i = 0; i < orderStates.length; i++) {
              if (orderStates[i].stateId == stateId) {
                orderStates.splice(i, 1)
                break
              }
            }
            self.setData({ orderStates: orderStates })
          })
        } else if (sm.cancel) {
          console.log('用户点击取消')
        }
