...
public class ProductController extends BaseController {
	...
	public void list(@ModelAttribute("productClassObj") ProductClass productClassObj,String productName,String recommendFlag,String addTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
	...
	public String frontlist(@ModelAttribute("productClassObj") ProductClass productClassObj,String productName,String recommendFlag,String addTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (productName == null) productName = "";
		if (recommendFlag == null) recommendFlag = "";
		if (addTime == null) addTime = "";
		List<Product> productList = productService.queryProduct(productClassObj, productName, recommendFlag, addTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    productService.queryTotalPageAndRecordNumber(productClassObj, productName, recommendFlag, addTime);
	    /*获取到总的页码数目*/
	    int totalPage = productService.getTotalPage();
	    ...
	    request.setAttribute("recommendFlag", recommendFlag);
	...
	public void OutToExcel(@ModelAttribute("productClassObj") ProductClass productClassObj,String productName,String recommendFlag,String addTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(productName == null) productName = "";
        if(recommendFlag == null) recommendFlag = "";
        if(addTime == null) addTime = "";
        List<Product> productList = productService.queryProduct(productClassObj,productName,recommendFlag,addTime);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Product信息记录"; 
        String[] headers = { "商品id","商品类别","商品名称","商品主图","商品价格","商量数量","是否推荐","人气值","上架时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<productList.size();i++) {
        	Product product = productList.get(i); 
        	dataset.add(new String[]{product.getProductId() + "",product.getProductClassObj().getClassName(),product.getProductName(),product.getMainPhoto(),product.getPrice() + "",product.getProductCount() + "",product.getRecommendFlag(),product.getHotNum() + "",product.getAddTime()});
        }
        /*
